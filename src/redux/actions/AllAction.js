...
function _requestObj(opt) {
    ...
    return new Request('http://gank.io/api/data/' + opt.type + '/15/' + opt.num
...
function android_doing(data, isRefreshing, isLoadging) {
    return {
        type: Type.all.ALL_DOING, data: data,
        isRefreshing: isRefreshing, isLoading: isLoadging
    }
}

function android_done(data) {
    return {type: Type.all.ALL_DONE, data: data}
}

function android_error(data) {
    return {type: Type.all.ALL_ERROR, data: data}
}